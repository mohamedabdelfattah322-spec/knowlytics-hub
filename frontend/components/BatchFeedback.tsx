...
export default function BatchFeedback({
  batchId, kind, totalSessions, recordingsCount,
}: {
  batchId: string;
  kind: 'first' | 'last';
  ...
  useEffect(() => {
    api.get(`/batches/${batchId}/my-feedback`)
      .then(({ data }) => {
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      await api.post(`/batches/${batchId}/feedback`, { kind, ...form });
  ...
  // If already submitted, show small badge
  if (existing && !open) {
    return (
  ...
  // If no prompt and no existing → don't show anything
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-white font-medium">
                {kind === 'first' ? '👋 رأيك يهمنا — فيدباك بداية الكورس' : '🎓 الكورس قارب على الانتهاء — قيّم تجربتك'}
              </p>
  ...
  return (
    <div className="card space-y-4 border-brand-500/30">
      ...
      {kind === 'first' ? (
        ...
        <>
          <div>
            <label className="block text-sm text-slate-300 mb-1.5">إيه أكتر حاجة استفدت منها في الكورس؟</label>
